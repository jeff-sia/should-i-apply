...
if not st.session_state.consent:
    ...
    if st.button("🚀 Run Analysis", type="primary", use_container_width=True):
        if not st.session_state.jd_text or not st.session_state.resume_text:
            st.error("Missing inputs: Both documents are required.")
        else:
            prompt = f"Evaluate RESUME against JD. Rubric: H(50%), I(30%), V(20%). JD: {st.session_state.jd_text} RESUME: {st.session_state.resume_text}"
            ...
            if not success and (mode == "OpenAI" or openai_key) and openai_key:
                try:
                    with st.spinner("Analyzing via OpenAI..."):
                        oa_client = OpenAI(api_key=openai_key)
                        completion = oa_client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": prompt}])
            ...
            if not success and or_key:
                try:
                    with st.spinner("Analyzing via OpenRouter..."):
                        ...
                        completion = or_client.chat.completions.create(model="anthropic/claude-3.5-sonnet", messages=[{"role": "user", "content": prompt}])
                        st.markdown(completion.choices[0].message.content)
                        success = True
                except Exception as e:
                    st.error(f"Critical: All engines failed. {e}")
